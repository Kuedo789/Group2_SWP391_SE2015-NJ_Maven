            ...
            <div class="main-panel">
                <div class="top-header">
                    <div class="header-left d-flex align-items-center gap-3">
                        <button class="sidebar-toggle btn p-0 border-0 fs-5 text-secondary"><i
                        ...
                        <div class="profile-section">
                            ...
                            <div class="profile-info" style="line-height: 1.2;">
                                <div class="profile-name" style="font-size: 13.5px; font-weight: 600; color: #333;">
                                    <c:out
                                        value="${not empty sessionScope.user.fullName ? sessionScope.user.fullName : 'Hoàng Anh'}" />
                                </div>
                                <div class="profile-role"
                                    style="font-size: 10.5px; color: var(--cz-text-muted); font-weight: 500;">Quản trị
                                    viên</div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="content-container">
                    <div class="form-card">
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            ...
                            <button type="button" class="btn btn-cz-primary" data-bs-toggle="modal"
                                data-bs-target="#addFeatureModal">
                                <i class="fa-solid fa-circle-plus"></i> Khai báo tính năng mới
                            </button>
                        </div>
                        <p class="page-subtitle">Bật / Tắt trạng thái để cấu hình real-time danh mục quyền hạn truy cập
                            của các nhóm nhân sự</p>
                            ...
                            <c:choose>
                                <c:when test="${not empty SCREEN_LIST}">
                                    ...
                                    <c:forEach items="${SCREEN_LIST}" var="s">
                                        <c:set var="groupName"
                                            value="${s.screenName.contains(':') ? s.screenName.split(':')[0] : 'Tính năng khác'}" />
                                        ...
                                        <div class="feature-matrix-row" id="row-${s.screenId}">
                                            <div
                                                class="feature-meta-info ${isMainFeature ? 'feature-main' : 'feature-sub'}">
                                                <i class="fa-solid ${isMainFeature ? 'fa-star' : 'fa-caret-right'}"></i>
                                                <span>${s.screenName.contains(':') ? s.screenName.split(':')[1] :
                                                    s.screenName}</span>
                                            </div>
                                            ...
                                            <div class="d-flex align-items-center gap-2">
                                                <div class="toggle-switch-box">
                                                    <button type="button"
                                                        onclick="updatePermission('${CURRENT_ROLE_ID}', '${s.screenId}', 'on')"
                                                        class="toggle-btn btn-on ${s.activated ? 'on-active' : ''}">On</button>
                                                    <button type="button"
                                                        onclick="updatePermission('${CURRENT_ROLE_ID}', '${s.screenId}', 'off')"
                                                        class="toggle-btn btn-off ${!s.activated ? 'off-active' : ''}">Off</button>
                                                </div>

                                                <button type="button" class="btn-action-edit"
                                                    onclick="openEditModal('${s.screenId}', '${s.screenName}', '${s.endpointUrl}')"
                                                ...
                                                <a href="${pageContext.request.contextPath}/admin/role-permissions?action=delete-feature&id=${s.screenId}&roleId=${CURRENT_ROLE_ID}"
                                                    ...
                                                    onclick="return confirm('Bạn có chắc chắn muốn ẩn vĩnh viễn tính năng này khỏi danh mục quản trị không?')"
                                                    title="Xóa mềm tính năng">
                            ...
                            <div class="mt-5 border-top pt-4">
                                ...
                                <div class="table-card p-3 mt-3 bg-light border">
                                    <c:forEach items="${HIDDEN_SCREEN_LIST}" var="hs">
                                        <div
                                            class="d-flex justify-content-between align-items-center p-2 border-bottom last-border-0">
                    ...
                    <div class="modal-content"
                        ...
                        <form action="${pageContext.request.contextPath}/admin/role-permissions" method="POST">
                            ...
                            <div class="modal-header"
                                style="background-color: #f1ede8; border-top-left-radius: 12px; border-top-right-radius: 12px;">
                                <h5 class="modal-title fw-bold" style="color: var(--cz-primary); font-size: 16px;"><i
                                        class="fa-solid fa-folder-plus me-2"></i>Khai báo tính năng hệ thống mới</h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal"
                                    aria-label="Close"></button>
                            </div>
                            <div class="modal-body p-4" style="font-size: 14px;">
                                <div class="mb-3">
                                    <label class="form-label fw-bold mb-1">Mã tính năng (Screen ID) <span
                                            class="text-danger">*</span></label>
                                    <input type="text" name="screenId" class="form-control"
                                        placeholder="Ví dụ: FEAT_VOUCHER_VIEW"
                                        style="text-transform: uppercase; height: 44px;" required>
                                </div>
                                <div class="mb-3">
                                    <label class="form-label fw-bold mb-1">Tên hiển thị phân nhóm <span
                                            class="text-danger">*</span></label>
                                    <input type="text" name="screenName" class="form-control"
                                        placeholder="Ví dụ: Khuyến mãi:Xem danh sách" style="height: 44px;" required>
                                    <div class="text-muted mt-1" style="font-size: 12px;">Cú pháp bắt buộc: <b
                                            class="text-dark">Tên Nhóm:Tên Chức Năng</b></div>
                                </div>
                                <div class="mb-2">
                                    <label class="form-label fw-bold mb-1">Đường dẫn bảo mật (Endpoint URL) <span
                                            class="text-danger">*</span></label>
                                    <input type="text" name="endpointUrl" class="form-control"
                                        placeholder="Ví dụ: /admin/voucher?action=list" style="height: 44px;" required>
                                </div>
                            </div>
                            <div class="modal-footer" style="background-color: #faf9f7;">
                                <button type="button" class="btn btn-sm btn-secondary px-3" style="height: 36px;"
                                    data-bs-dismiss="modal">Hủy bỏ</button>
                                <button type="submit" class="btn btn-sm btn-cz-primary px-3" style="height: 36px;"><i
                                        class="fa-solid fa-floppy-disk me-1"></i>Khai báo</button>
                            </div>
                        </form>
                    </div>
                </div>
                    ...
                    <div class="modal-content"
                        ...
                        <form action="${pageContext.request.contextPath}/admin/role-permissions" method="POST">
                            ...
                            <div class="modal-header"
                                style="background-color: #f1ede8; border-top-left-radius: 12px; border-top-right-radius: 12px;">
                                <h5 class="modal-title fw-bold" style="color: var(--cz-primary); font-size: 16px;"><i
                                        class="fa-regular fa-pen-to-square me-2"></i>Chỉnh sửa cấu hình tính năng</h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal"
                                    aria-label="Close"></button>
                            </div>
                            <div class="modal-body p-4" style="font-size: 14px;">
                                <div class="mb-3">
                                    ...
                                    <input type="text" id="editScreenId" name="screenId" class="form-control bg-light"
                                        style="height: 44px; font-weight: 600;" readonly>
                                </div>
                                <div class="mb-3">
                                    <label class="form-label fw-bold mb-1">Tên hiển thị phân nhóm <span
                                            class="text-danger">*</span></label>
                                    <input type="text" id="editScreenName" name="screenName" class="form-control"
                                ...
                                <div class="mb-2">
                                    <label class="form-label fw-bold mb-1">Đường dẫn bảo mật (Endpoint URL) <span
                                            class="text-danger">*</span></label>
                                    <input type="text" id="editEndpointUrl" name="endpointUrl" class="form-control"
                                        style="height: 44px;" required>
                                </div>
                            </div>
                            <div class="modal-footer" style="background-color: #faf9f7;">
                                <button type="button" class="btn btn-sm btn-secondary px-3" style="height: 36px;"
                                    data-bs-dismiss="modal">Hủy bỏ</button>
                                <button type="submit" class="btn btn-sm btn-cz-primary px-3" style="height: 36px;"><i
                                        class="fa-solid fa-floppy-disk me-1"></i>Lưu thay đổi</button>
                            </div>
                        </form>
                    </div>
                </div>
