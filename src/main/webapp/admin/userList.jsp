                ...
                <div class="filter-card">
                    <form class="filter-form" action="${pageContext.request.contextPath}/userList" method="GET">
                        ...
                        <select class="filter-select" name="filterStatus" onchange="this.form.submit()">
                            ...
                            <option value="Active" ${param.filterStatus == 'Active' ? 'selected' : ''}>Đang hoạt động</option>
                            <option value="Deactive" ${param.filterStatus == 'Deactive' ? 'selected' : ''}>Đã khóa</option>
                        </select>
                    ...
                    <table class="cz-table">
                        ...
                        <tbody>
                            <c:forEach items="${USERS}" var="u" varStatus="status">
                                <tr>
                                    <td class="text-center">${(currentPage - 1) * 5 + status.count}</td>

                                    <td class="fw-bold">${u.fullName}</td>

                                    <td>${u.email}</td>

                                    <td class="text-warning fw-medium">${u.phone}</td>

                                    <td class="text-center">
                                        <c:set var="userRole" value="${not empty u.roleId ? u.roleId : u.role_ID}" />

                                        <c:choose>
                                            <c:when test="${userRole eq 'ADMIN'}">
                                                <span class="badge badge-admin">Quản lý</span>
                                            </c:when>
                                            <c:when test="${userRole eq 'STAFF'}">
                                                <span class="badge badge-staff">Nhân viên</span>
                                            </c:when>
                                            <c:when test="${userRole eq 'SHIPPER'}">
                                                <span class="badge badge-shipper">Người giao hàng</span>
                                            </c:when>
                                        </c:choose>
                                    </td>

                                    <td class="text-center">
                                        <c:set var="userStatus" value="${not empty u.accountStatus ? u.accountStatus : u.account_Status}" />

                                        <c:choose>
                                            <c:when test="${userStatus eq 'Active'}">
                                                <span class="status-badge-active">Đang hoạt động</span>
                                            </c:when>
                                            <c:when test="${userStatus eq 'Deactive'}">
                                    ...
                                    <td>
                                        <div class="actions-cell justify-content-center">
                                            <c:set var="currentId" value="${not empty u.staffId ? u.staffId : u.staff_ID}" />

                                            <a href="userDetail?action=edit&id=${currentId}" class="btn-action-edit" title="Chỉnh sửa">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                            <a href="userDetail?action=delete&id=${currentId}" class="btn-action-delete" 
