<%-- Document : voucher-management Created on : Jul 9, 2026 Author : thais / antigravity --%>
                ...
                <body>
                    ...
                    <div class="main-panel">
                        ...
                        <div class="content">
                            ...
                            <c:if test="${not empty success}">
                                <div
                                    style="background:#dcfce7;color:#166534;padding:12px 20px;border-radius:8px;margin-bottom:20px;border:1px solid #bbf7d0;">
                                    <i class="fa-solid fa-circle-check"></i>
                                    <c:choose>
                                        <c:when test="${success == 'created'}">Voucher mới đã được tạo thành công!
                                        </c:when>
                                        <c:when test="${success == 'deleted'}">Voucher đã bị xóa thành công.</c:when>
                                        <c:when test="${success == 'deactivated'}">Voucher đã được vô hiệu hóa (có dữ
                                            liệu liên kết nên không thể xóa hoàn toàn).</c:when>
                                        <c:when test="${success == 'activated'}">Voucher đã được kích hoạt lại.</c:when>
                                        <c:otherwise>Thao tác thành công!</c:otherwise>
                                    </c:choose>
                                </div>
                            ...
                            <c:if test="${not empty error}">
                                <div
                                    ...
                                    <strong>Lỗi:</strong>
                                    <c:choose>
                                        <c:when test="${error == 'delete_failed'}">Không thể xóa voucher này. Có thể đã
                                            được gán cho người dùng.</c:when>
                                        <c:when test="${error == 'toggle_failed'}">Không thể cập nhật trạng thái
                                            voucher. Vui lòng thử lại.</c:when>
                            ...
                            <div class="table-card">
                                ...
                                <div class="filter-tabs-container">
                                    <a href="${pageContext.request.contextPath}/admin/vouchers?status=all&search=${searchQuery}"
                                        ...
                                        <c:if test="${statusFilter == 'all' || empty statusFilter}">
                                            <span
                                                style="margin-left:5px;background:rgba(255,255,255,0.3);padding:1px 7px;border-radius:20px;font-size:11px;">${totalRecords}</span>
                                        </c:if>
                                    </a>
                                    <a href="${pageContext.request.contextPath}/admin/vouchers?status=ACTIVE&search=${searchQuery}"
                                        ...
                                        <i class="fa-solid fa-circle"
                                            style="font-size:8px;vertical-align:middle;margin-right:4px;color:${statusFilter == 'ACTIVE' ? '#fff' : '#16a34a'};"></i>
                                        Đang hoạt động
                                    ...
                                    <a href="${pageContext.request.contextPath}/admin/vouchers?status=EXPIRED&search=${searchQuery}"
                                        ...
                                        <i class="fa-solid fa-circle"
                                            style="font-size:8px;vertical-align:middle;margin-right:4px;color:${statusFilter == 'EXPIRED' ? '#fff' : '#64748b'};"></i>
                                        Hết hạn
                                    ...
                                    <a href="${pageContext.request.contextPath}/admin/vouchers?status=INACTIVE&search=${searchQuery}"
                                        ...
                                        <i class="fa-solid fa-circle"
                                            style="font-size:8px;vertical-align:middle;margin-right:4px;color:${statusFilter == 'INACTIVE' ? '#fff' : '#ea580c'};"></i>
                                        Vô hiệu hóa
                                ...
                                <div class="table-controls">
                                    <form action="${pageContext.request.contextPath}/admin/vouchers" method="GET"
                                        ...
                                        <button type="submit" class="btn-primary" style="padding:10px 20px;">Tìm
                                            kiếm</button>
                                        ...
                                        <c:choose>
                                            ...
                                            <c:otherwise>
                                                <c:forEach var="v" items="${vouchers}" varStatus="loop">
                                                    ...
                                                    <tr>
                                                        <td style="color:var(--text-muted);font-size:13px;">${loop.index
                                                            + 1}</td>

                                                        <td>
                                                            <code
                                                                style="background:#f1f5f9;padding:3px 8px;border-radius:6px;font-size:12px;font-weight:600;letter-spacing:0.5px;">
                                                    ${v.voucherCode}
                                                </code>
                                                        </td>

                                                        <td style="max-width:180px;">
                                                            <div style="font-weight:600;font-size:14px;">${v.title}
                                                            </div>
                                                        ...
                                                        <td>
                                                            <c:choose>
                                                                <c:when test="${v.discountType == 'PERCENT'}">
                                                                    <span class="badge chip-percent">Phần trăm
                                                                        (%)</span>
                                                                </c:when>
                                                                <c:otherwise>
                                                        ...
                                                        <td>
                                                            <c:choose>
                                                                <c:when
                                                                    test="${v.minOrderValue != null && v.minOrderValue > 0}">
                                                                    <fmt:formatNumber value="${v.minOrderValue}"
                                                                        type="number" groupingUsed="true" />₫
                                                                </c:when>
                                                                <c:otherwise>
                                                                    <span
                                                                        style="color:var(--text-muted);font-size:12px;">Không
                                                                        giới hạn</span>
                                                                </c:otherwise>
                                                            </c:choose>
                                                        </td>
                                                        ...
                                                        <td>
                                                            ...
                                                            <c:if test="${v.endDate != null}">
                                                                <fmt:formatDate value="${v.endDate}"
                                                                    pattern="dd/MM/yyyy" />
                                                            </c:if>
                                                        </td>

                                                        <td>
                                                            <%-- Use a scriptlet-computed string stored as a request
                                                                attribute because EL cannot call java.sql.Date.before()
                                                                reliably across all servers. We check: active=true and
                                                                ...
                                                                endDate < today=> EXPIRED
                                                                    ...
                                                                    <c:choose>
                                                                        <c:when test="${!v.active}">
                                                                            <span class="badge badge-inactive">Vô hiệu
                                                                                hóa</span>
                                                                        </c:when>
                                                                        <c:when test="${v.active}">
                                                                            <%-- We use a JSP fragment to compute expiry
                                                                                since EL can't do date math --%>
                                                                                <% com.bakeryzone.model.Voucher
                                                                                    _vTmp=(com.bakeryzone.model.Voucher)
                                                                                    pageContext.getAttribute("v");
                                                                                    boolean _expired=false; if (_vTmp
                                                                                    !=null && _vTmp.getEndDate() !=null)
                                                                                    {
                                                                                    _expired=_vTmp.getEndDate().before(new
                                                                                    java.util.Date()); }
                                                                                    pageContext.setAttribute("isExpiredNow",
                                                                                    _expired); %>
                                                                                    <c:choose>
                                                                                        <c:when test="${isExpiredNow}">
                                                                                            <span
                                                                                                class="badge badge-expired">Hết
                                                                                                hạn</span>
                                                                                        </c:when>
                                                                                        <c:otherwise>
                                                                                            <span
                                                                                                class="badge badge-active">Đang
                                                                                                hoạt động</span>
                                                                                        </c:otherwise>
                                                                                    </c:choose>
                                                                        </c:when>
                                                                        <c:otherwise>
                                                                            <span class="badge badge-expired">Hết
                                                                                hạn</span>
                                                                        </c:otherwise>
                                                                    </c:choose>
                                                        </td>
...
>>>>>>> Stashed changes
                                                ...
                                                <%-- Sliding Window Calculations --%>
                                                    ...
                                                    <%-- Safety check: Ensure the end range doesn't exceed total page
                                                        bounds --%>
                                                        <c:if test="${endPage > totalPages}">
                                                            <c:set var="endPage" value="${totalPages}" />
                                                            <%-- If we are on the very last page, adjust the start page
                                                                back by 1 so 2 numbers still render if possible --%>
                                                        ...
                                                        <%-- Render the Sliding Active Window --%>
                                                            ...
                                                            <%-- Append Ellipsis and the Later Block bounds if more
                                                                pages remain outside our active window --%>
                                                                <c:if test="${endPage < totalPages}">
                                                                    <span class="pagination-ellipsis disabled"
                                                                        style="padding: 6px 8px; color: #718096; border:none; background:none; cursor:default;">...</span>

                                                                    <%-- Render the final remaining page grouping
                                                                        calculations if relevant --%>
                                                                        <c:if test="${endPage < totalPages - 1}">
                                                                            <a href="${pageContext.request.contextPath}/admin/vouchers${pageQueryBase}&page=${totalPages - 1}"
                                                                                class="page-btn"
                                                                                style="text-decoration:none;">${totalPages
                                                                                - 1}</a>
                                                                        </c:if>
                                                                        <a href="${pageContext.request.contextPath}/admin/vouchers${pageQueryBase}&page=${totalPages}"
                                                                            class="page-btn"
                                                                            style="text-decoration:none;">${totalPages}</a>
                                                                </c:if>
