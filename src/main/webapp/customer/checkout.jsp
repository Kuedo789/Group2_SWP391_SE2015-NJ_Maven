        ...
        <%@ taglib prefix="fmt" uri="jakarta.tags.fmt" %>
            ...
            <body>
                ...
                <main class="checkout-container">
                    ...
                    <div id="jsErrorBox" class="alert alert-danger"
                        style="display: none; color: #842029; background-color: #f8d7da; border-color: #f5c2c7; padding: 1rem 1rem; margin-bottom: 1rem; border: 1px solid transparent; border-radius: .25rem; max-width: 1200px; margin-left: auto; margin-right: auto;">
                    </div>
                    ...
                    <c:if test="${param.error == 'empty_cart'}">
                        <div class="alert alert-danger"
                            style="color: #842029; background-color: #f8d7da; border-color: #f5c2c7; padding: 1rem 1rem; margin-bottom: 1rem; border: 1px solid transparent; border-radius: .25rem; max-width: 1200px; margin-left: auto; margin-right: auto;">
                    ...
                    <c:if
                        test="${not empty param.error && param.error != 'empty_cart' && param.error != 'empty_address' && param.error != 'server_error' && param.error != 'save_failed' && param.error != 'note_too_long' && param.error != 'voucher_invalid'}">
                    ...
                    <form id="checkoutForm" action="${pageContext.request.contextPath}/checkout" method="POST">
                        ...
                        <input type="hidden" name="addressId" id="selectedAddressIdInput"
                            value="${requestScope.selectedAddress.addressId}">
                            ...
                            <div class="checkout-left-col">
                                ...
                                <div class="checkout-card">
                                    <div class="card-header">
                                        ...
                                        <a href="${pageContext.request.contextPath}/delivery-address?source=checkout"
                                            class="btn-add-address"
                                            style="text-decoration: none; font-size: 14px; font-weight: 700; color: var(--primary-dark); display: flex; align-items: center; gap: 6px;">
                                        ...
                                        <c:choose>
                                            <c:when test="${empty requestScope.addressList}">
                                                <div class="empty-address-msg"
                                                    style="text-align: center; padding: 30px 16px; border: 1px dashed var(--border-color); border-radius: var(--radius-md);">
                                                    <i class="fa fa-map-marked-alt"
                                                        style="font-size: 40px; color: #ddd; margin-bottom: 16px;"></i>
                                                    <p style="color: var(--text-muted); margin-bottom: 16px;">Bạn chưa
                                                        lưu địa chỉ giao hàng nào.</p>
                                                    <a href="${pageContext.request.contextPath}/delivery-address?action=add&source=checkout"
                                            ...
                                            <c:otherwise>
                                                <div class="address-card-option active" id="finalSelectedAddressCard"
                                                    ...
                                                    <div class="address-details"
                                                        ...
                                                        <div class="address-name-tag"
                                                            style="font-weight: 700; font-size: 16px; color: var(--text-dark); display: flex; align-items: center; gap: 8px;">
                                                            ${requestScope.selectedAddress.receiverName}
                                                            <c:if test="${requestScope.selectedAddress.isDefault()}">
                                                                <span class="default-badge"
                                                                    style="font-size: 11px; font-weight: 600; color: var(--primary-dark); background-color: #e2ece5; padding: 2px 8px; border-radius: 4px;">Mặc
                                                                    định</span>
                                                            </c:if>
                                                        </div>
                                                        <div class="address-text"
                                                            style="font-size: 14px; color: var(--text-muted); line-height: 1.5; margin-bottom: 6px;">
                                ...
                                <div class="checkout-card">
                                    ...
                                    <div style="margin-bottom: 24px; display: flex; flex-direction: column; gap: 8px;">
                                        <label for="deliveryDate"
                                            style="font-weight: 700; font-size: 14px; color: var(--text-dark);">Chọn
                                            ngày giao hàng: <span style="color:#d62828;">*</span></label>
                                        <input type="date" id="deliveryDate" name="deliveryDate"
                                            style="width: 100%; height: 48px; padding: 0 16px; border: 1px solid var(--border-color); border-radius: var(--radius-md); font-family: var(--font-body); font-size: 15px; outline: none; box-sizing: border-box;"
                                            required>
                                    </div>

                                    <div
                                        style="font-weight: 700; font-size: 14px; color: var(--text-dark); margin-bottom: 8px;">
                                        Chọn khung giờ: <span style="color:#d62828;">*</span></div>
                                        ...
                                        <c:if test="${empty checkoutCartItems}">
                                            <div class="empty-cart-state"
                                                ...
                                                <i class="fa fa-shopping-basket"
                                                    style="font-size: 32px; margin-bottom: 12px; display: block; color: #ccc;"></i>
                                                <p>Không có sản phẩm nào được chọn để thanh toán.</p>
                                                <a href="${pageContext.request.contextPath}/cart"
                                                    class="btn btn-primary"
                                                    style="margin-top:10px; display:inline-block; padding:8px 16px; background:var(--primary-dark); color:white; border-radius:8px; text-decoration:none;">Quay
                                                    lại giỏ hàng</a>
                                            </div>
                                        </c:if>
                                        <c:forEach var="item" items="${checkoutCartItems}">
                                            <div class="product-checkout-item"
                                                style="display: flex; gap: 12px; padding: 16px; border-bottom: 1px solid var(--border-color);">
                                                <img src="${item.imageUrl}" alt="${item.name}"
                                                    style="width: 70px; height: 70px; border-radius: 8px; object-fit: cover; flex-shrink: 0; border: 1px solid var(--border-color);">
                                                <div
                                                    style="flex: 1; display: flex; flex-direction: column; justify-content: center;">
                                                    <div
                                                        style="font-weight: 700; font-size: 15px; color: var(--text-dark); margin-bottom: 4px;">
                                                        ${item.name}</div>
                                                    <div
                                                        style="font-size: 13px; color: var(--text-muted); display: flex; justify-content: space-between; align-items: center;">
                                                        <span>Số lượng: <strong>${item.quantity}</strong></span>
                                                        <span
                                                            style="font-weight: 700; color: var(--text-dark); font-size: 15px;">
                                ...
                                <div class="checkout-card">
                                    ...
                                    <div style="margin-top: 16px;">
                                        <textarea name="note" id="note" rows="3" maxlength="100"
                                            oninput="updateNoteCounter()"
                                            placeholder="Ví dụ: Giao hàng giờ hành chính, gọi điện trước khi giao..."
                                            style="width: 100%; padding: 12px 16px; border: 1px solid var(--border-color); border-radius: var(--radius-md); font-family: var(--font-body); font-size: 15px; outline: none; box-sizing: border-box; resize: vertical;"></textarea>
                                        <div style="display:flex; justify-content:flex-end; margin-top:4px;">
                                            <span id="noteCharCount"
                                                style="font-size:12px; color:var(--text-muted);">0/100 ký tự</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="checkout-card">
                                    ...
                                    <div class="payment-methods-wrapper" style="margin-top: 16px;">
                                        <label class="pm-card active"
                                            onclick="document.querySelectorAll('.pm-card').forEach(c => c.classList.remove('active')); this.classList.add('active');">
                                            <input type="radio" name="paymentMethod" value="COD" checked>
                                            <div class="pm-radio-circle"></div>
                                            <i class="fa fa-truck pm-icon"></i>
                                            <span class="pm-title">Thanh toán khi nhận hàng <span
                                                    style="font-size: 13px; color: #d9534f; font-weight: 600;">(Cọc
                                                    ${not empty settings.depositPercent ? settings.depositPercent :
                                                    '30'}%)</span></span>
                                        </label>
                            ...
                            <div>
                                <div class="summary-sidebar">
                                    ...
                                    <div class="summary-row">
                                        ...
                                        <span id="productTotalSum" data-val="${productTotalSum}">
                                            <fmt:formatNumber value="${productTotalSum}" type="currency"
                                    ...
                                    <div class="summary-row">
                                        <div class="summary-row-label">
                                            ...
                                            <span class="summary-sub-label">Tính dựa trên tọa độ khoảng cách (
                                                <fmt:formatNumber
                                                    value="${not empty settings.shippingRate ? settings.shippingRate : 5000}"
                                                    pattern="#,##0" />đ/km)
                                            </span>
                                        </div>
                                    ...
                                    <div id="discountContainer">
                                        <c:if test="${requestScope.checkoutOrderDiscount > 0}">
                                            <div class="summary-row" id="discountSummaryRow" style="color: #d9534f;">
                                                ...
                                                <span>-
                                                    <fmt:formatNumber value="${requestScope.checkoutOrderDiscount}"
                                                        type="currency" currencySymbol="₫" maxFractionDigits="0" />
                                                </span>
                                            </div>
                                        </c:if>
                                        <c:if test="${not empty requestScope.checkoutShippingVoucherCode}">
                                            <div class="summary-row" id="shippingDiscountSummaryRow"
                                                style="color: #d9534f;">
                                                <div class="summary-row-label">
                                    ...
                                    <div class="voucher-group" id="voucherGroupContainer" style="margin-bottom: 24px;">
                                        <div class="voucher-group" id="voucherGroupContainer"
                                            style="margin-bottom: 24px;">
                                            <div
                                                style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
                                                <span
                                                    style="font-weight: 700; font-size: 14px; color: var(--text-dark);">Voucher
                                                    đã áp dụng</span>
                                                <a href="${pageContext.request.contextPath}/cart"
                                                    style="font-size: 13px; color: var(--primary); text-decoration: none; font-weight: 600;">Thay
                                                    đổi</a>
                                            </div>
                                            ...
                                            <div style="display: flex; flex-direction: column; gap: 8px;">
                                                <div style="display: flex; gap: 8px; margin-bottom: 8px;">
                                                    <input type="text" id="manualVoucherInput"
                                                        ...
                                                        style="flex: 1; padding: 8px 12px; border: 1px solid var(--border-color); border-radius: 6px; font-size: 14px; outline: none;">
                                                    <button type="button" onclick="applyManualVoucher()"
                                                        style="padding: 8px 16px; background: var(--primary); color: white; border: none; border-radius: 6px; cursor: pointer; font-weight: 600; font-size: 14px;">Áp
                                                        dụng</button>
                                                </div>
                                                <div id="manualVoucherError"
                                                    style="color: #d9534f; font-size: 13px; display: none; margin-bottom: 8px;">
                                                </div>
                                                <div id="appliedVouchersList"
                                                    style="display: flex; flex-direction: column; gap: 8px;">
                                                    <c:if
                                                        test="${empty requestScope.checkoutOrderVoucherCode && empty requestScope.checkoutShippingVoucherCode}">
                                                        <div id="noVoucherText"
                                                            style="font-size: 13px; color: var(--text-muted); font-style: italic;">
                                                            Chưa có voucher nào được áp dụng.</div>
                                                    </c:if>
                                                    <c:if test="${not empty requestScope.checkoutOrderVoucherCode}">
                                                        <div class="applied-voucher-item"
                                                            data-code="${requestScope.checkoutOrderVoucherCode}"
                                                            data-scope="ORDER" data-type="PERCENT"
                                                            data-val="${requestScope.checkoutOrderDiscount}"
                                                            data-max="0"
                                                            style="display: flex; justify-content: space-between; align-items: center; width: 100%; padding: 8px 12px; background: #f3f7f2; border: 1px dashed var(--primary); border-radius: 8px;">
                                                            <div style="display: flex; align-items: center; gap: 6px;">
                                                                <span
                                                                    style="font-size: 12px; background: var(--primary); color: white; padding: 2px 6px; border-radius: 4px; font-weight: 700;">ĐƠN
                                                                    HÀNG</span>
                                                                <span
                                                                    style="font-weight: 600; color: var(--primary);">${requestScope.checkoutOrderVoucherCode}</span>
                                                            </div>
                                                            <button type="button" onclick="removeVoucher('ORDER')"
                                                                style="background: none; border: none; color: #d9534f; cursor: pointer;"><i
                                                                    class="fa fa-times"></i></button>
                                                        </div>
                                                    </c:if>
                                                    <c:if test="${not empty requestScope.checkoutShippingVoucherCode}">
                                                        <div class="applied-voucher-item"
                                                            data-code="${requestScope.checkoutShippingVoucherCode}"
                                                            data-scope="SHIPPING"
                                                            data-type="${requestScope.shippingVoucherType}"
                                                            data-val="${requestScope.shippingVoucherValue}"
                                                            data-max="${requestScope.shippingVoucherMax}"
                                                            style="display: flex; justify-content: space-between; align-items: center; width: 100%; padding: 8px 12px; background: #f3f7f2; border: 1px dashed var(--primary); border-radius: 8px;">
                                                            <div style="display: flex; align-items: center; gap: 6px;">
                                                                <span
                                                                    style="font-size: 12px; background: #4caf50; color: white; padding: 2px 6px; border-radius: 4px; font-weight: 700;">VẬN
                                                                    CHUYỂN</span>
                                                                <span
                                                                    style="font-weight: 600; color: var(--primary);">${requestScope.checkoutShippingVoucherCode}</span>
                                                            </div>
                                                            <button type="button" onclick="removeVoucher('SHIPPING')"
                                                                style="background: none; border: none; color: #d9534f; cursor: pointer;"><i
                                                                    class="fa fa-times"></i></button>
                                                        </div>
                                                    </c:if>
                                        ...
                                        <div class="banner-kitchen-verify">
                                            ...
                                            <span>Bếp đã xác nhận đủ năng lực sản xuất cho các sản phẩm và khung giờ
                                                được chọn.</span>
                                        </div>
